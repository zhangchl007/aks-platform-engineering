...
| Platform demo ArgoCD apps | `kubectl --context gitops-aks-admin -n argocd get applications -l app.kubernetes.io/part-of=platform-demo` | `platform-demo-aks-gitops-aks`, `platform-demo-kind-arc-demo-vm`, and `platform-demo-kind-arc-demo-vm-2` Synced / Healthy; this is the single default demo workload per Arc/kind cluster |
...
### Multiple Arc demo workload applications appear

Each Arc/kind cluster should have only one default preloaded demo workload:
`platform-demo-kind-<cluster>`. If ArgoCD also shows
`arc-baseline-arc-demo-vm` or `arc-baseline-arc-demo-vm-2`, those are legacy
sample workloads generated by the `addons-arc-onboarding` ApplicationSet.
...
kubectl --context gitops-aks-admin -n argocd get application `
  arc-baseline-arc-demo-vm,arc-baseline-arc-demo-vm-2,platform-demo-kind-arc-demo-vm,platform-demo-kind-arc-demo-vm-2 `
  --ignore-not-found `
...
kubectl --context gitops-aks-admin -n argocd get secret arc-demo-vm arc-demo-vm-2 `
  -o custom-columns=NAME:.metadata.name,ARC_DEMO_WORKLOAD:.metadata.labels.enable_arc_demo_workload,PLATFORM_ACCESS:.metadata.labels.platform_access_enabled,TYPE:.metadata.labels.platform_cluster_type
```

The current default is:

- keep `platform-demo-kind-arc-demo-vm` and `platform-demo-kind-arc-demo-vm-2`;
- generate `arc-baseline-*` only when the corresponding cluster Secret has
  `enable_arc_demo_workload=true`.

After ArgoCD syncs this Git change, `arc-baseline-*` should stop being recreated.
If old `arc-demo` namespace resources remain after the `arc-baseline-*`
Applications are gone, remove only that now-unmanaged legacy sample namespace:

```powershell
kubectl --context arc-demo-vm-admin delete namespace arc-demo --ignore-not-found
kubectl --context arc-demo-vm-2-admin delete namespace arc-demo --ignore-not-found
```
