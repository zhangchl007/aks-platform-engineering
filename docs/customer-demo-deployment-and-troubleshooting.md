...
The `role:resource-browser-options___` helper is also intentional. Devtron's
Kubernetes Resource Browser option service requires
`global-resource/update` before it returns cluster, namespace, and resource type
dropdown data. This is a Devtron UI/API prerequisite, not the namespace write
authorization. Keep actual create/update scope on `clusterEdit_<cluster>_<namespace>`
and target-cluster Kubernetes RBAC.
...
If a user still cannot see the expected Pods after convergence, verify that the
current SSO token actually contains the expected Entra group object ID. Devtron
persists received group claims in `user_auto_assigned_groups`; if the row is
missing for the signed-in user, fix the Entra group membership or have the user
sign out and sign back in so Dex/Devtron refreshes group claims.
...
repo owns the desired shape, while private tenant values come from cluster-local
inputs:
...
metadata:
  labels:
    ...
    platform_cluster_type: aks # or kind
    platform_devtron_visibility: aks # or kind
  annotations:
    platform_k8sadmin_group_object_id: <private-k8sadmin-group-object-id>
    platform_backstage_catalog_enabled: "true"
```

For future AKS clusters, registration through
`scripts/register-aks-workload-cluster.ps1` sets the AKS labels. Re-running
`scripts/configure-k8sadmin-access.ps1` refreshes the private `k8sadmin` group
annotation across all registered clusters. `k8sadmin` receives cluster-admin on
registered targets; `akspe-aks-cluster-deployers` receives AKS target visibility
but deployment write access only where `platform-access-policy` explicitly lists
an approved namespace/environment.

The live ArgoCD `argocd-rbac-cm` must include a local-admin-equivalent
`k8sadmin` role generated by the ArgoCD Helm release, including:

```csv
g, <private-k8sadmin-group-object-id>, role:k8sadmin-admin
g, <private-k8sadmin-group-object-id>, role:admin
```

If the ArgoCD login page shows only the local `admin` form, or a `k8sadmin`
SSO user can log in but sees no applications/clusters, refresh the private
inputs and let ArgoCD reconcile the desired state:
...
2. resolves the private Entra group object IDs;
3. updates the private input Secret/annotations used by ArgoCD;
4. stores the ArgoCD OIDC client secret in `argocd-secret`;
5. refreshes the ArgoCD applications so GitOps reconciles `argocd-cm`,
   `argocd-rbac-cm`, target baseline RBAC, and the Devtron access convergence
   Job.
