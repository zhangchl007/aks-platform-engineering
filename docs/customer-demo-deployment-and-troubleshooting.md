...
| `argocd/gitops-aks` cluster Secret annotations | Private bootstrap | OIDC issuer/client ID and `k8sadmin` group object ID used by the ArgoCD add-on ApplicationSet |
...
5. refreshes the ArgoCD applications so GitOps reconciles `argocd-cm`,
   `argocd-rbac-cm`, and the Devtron access convergence Job.
