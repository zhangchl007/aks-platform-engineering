...
resource "azurerm_postgresql_flexible_server" "backstagedbserver" {
  ...
  name                          = "${var.resource_group_name}-backstage-postgresql-${var.postgres_location}"
...
resource "azuread_application" "backstage-app" {
  ...
  app_role {
    id                   = "af61329d-2857-f16b-2506-f6385c3e9ec0"
    allowed_member_types = ["User"]
  ...
  app_role {
    id                   = "30c0ef49-2dca-2bf6-312a-ffd07f017b65"
    allowed_member_types = ["User"]
...
resource "null_resource" "ascii_art" {
  count      = local.build_backstage && var.manage_backstage_entra_credentials ? 1 : 0
  depends_on = [azuread_application_password.backstage-app-password]
...
output "azure_client_secret" {
  ...
  value     = length(azuread_application_password.backstage-app-password) > 0 ? azuread_application_password.backstage-app-password[0].value : null
  sensitive = true
}
...
resource "azurerm_public_ip" "backstage_public_ip" {
  ...
  sku                 = var.backstage_public_ip_sku
